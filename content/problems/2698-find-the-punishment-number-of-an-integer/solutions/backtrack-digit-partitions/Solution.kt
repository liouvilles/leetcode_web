class Solution {
    fun punishmentNumber(n:Int):Int{
        fun canSplit(digits:String,index:Int,sum:Int,target:Int):Boolean{
            if(index==digits.length)return sum==target;
            var number=0;
            for(end in index until digits.length){
        ...
        for(value in 1..n){
            ...
            if(canSplit(square.toString(),0,0,value))answer+=square
        };
        return answer
    }
}
