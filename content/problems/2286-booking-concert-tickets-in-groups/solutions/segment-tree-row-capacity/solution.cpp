class BookMyShow {
    ...
    int find(int node,int left,int right,int maxRow,int k){
    ...
    long long query(int node,int left,int right,int maxRow){
    ...
    public:BookMyShow(int n,int m):n(n),m(m),remaining(n,m),maximum(4*n),sum(4*n){
        build(1,0,n-1);
    }
    ...
    bool scatter(int k,int maxRow){
        ...
        while(k){
            while(first<n&&!remaining[first])++first;
            int take=min(k,remaining[first]);
