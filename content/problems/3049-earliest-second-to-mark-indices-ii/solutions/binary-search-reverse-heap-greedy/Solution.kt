class Solution {
    fun earliestSecondToMarkIndices(nums:IntArray,changeIndices:IntArray):Int{
        var base=nums.size.toLong();
        for(value in nums)base+=value;
        fun canFinish(seconds:Int):Boolean{
            val first=IntArray(nums.size){
                -1
            };
            for(time in seconds-1 downTo 0)first[changeIndices[time]-1]=time;
            val chosen=java.util.PriorityQueue<Int>();
            var available=0;
            var saved=0L;
            for(time in seconds-1 downTo 0){
                ...
                if(time==first[index]&&value>1){
                    if(available>0){
                        ...
                        chosen.add(value);
                        saved+=value-1L
                    }else if(chosen.isNotEmpty()&&chosen.peek()<value){
                        val removed=chosen.poll();
                        saved-=removed-1L;
                        available++;
                        chosen.add(value);
                        saved+=value-1L
