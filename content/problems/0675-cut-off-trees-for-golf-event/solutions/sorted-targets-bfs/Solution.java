class Solution {
    public int cutOffTree(List<List<Integer>> forest){
        ...
        for(int r=0;r<forest.size();r++)for(int c=0;c<forest.get(0).size();c++)if(forest.get(r).get(c)>1)trees.add(new int[]{
            forest.get(r).get(c),r,c
        });
        ...
        while(!queue.isEmpty()){
            ...
            for(int size=queue.size();size>0;size--){
                ...
                for(int[] d:directions){
                    ...
                    if(r<0||r>=rows||c<0||c>=columns||seen[r][c]||forest.get(r).get(c)==0)continue;
