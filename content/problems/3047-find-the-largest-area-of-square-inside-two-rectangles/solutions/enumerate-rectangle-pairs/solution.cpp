class Solution {
    public:long long largestSquareArea(vector<vector<int>>& bottomLeft,vector<vector<int>>& topRight){
        long long answer=0;
        for(int i=0;i<(int)bottomLeft.size();++i)for(int j=i+1;j<(int)bottomLeft.size();++j){
