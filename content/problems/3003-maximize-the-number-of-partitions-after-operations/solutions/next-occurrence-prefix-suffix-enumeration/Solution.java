class Solution {
    private int n,limitDistinct;
    private int[][] next;
    private final int[] work=new int[26];
    private int cut(int start,int changedIndex,int oldLetter,int replacement){
        System.arraycopy(next[start],0,work,0,26);
    ...
    public int maxPartitionsAfterOperations(String s,int k){
        ...
        int[] segmentStart=new int[n],prefixPartitions=new int[n];
        int mask=0,start=0,parts=0;
        for(int i=0;i<n;i++){
            int bit=1<<(s.charAt(i)-'a');
            if(Integer.bitCount(mask|bit)>k){
                parts++;
                start=i;
                mask=bit;
            }else mask|=bit;
